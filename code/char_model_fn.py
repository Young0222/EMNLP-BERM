...
def format_features(features):  ## 使用q_t_neib_sparse标记[query, title, neighbor1, neighbor2, ..., neighbor8]
    name_list = ["q", "sna", "qiqiq1_t1", "qiqiq1_q2", "qiqiq2_t3", "qiqiq2_q4", "qiqiq1_t5", "qiqiq1_q6", "qiqiq2_t7", "qiqiq2_q8", "iqiqi1_q1", "iqiqi1_t2", "iqiqi2_q3", "iqiqi2_t4", "iqiqi1_q5", "iqiqi1_t6", "iqiqi2_q7", "iqiqi2_t8"]
    q_t_neib_sparse = []
    feature_name_unpad = []
    feature_name_dense = []
    for i in range(len(name_list)):
        q_t_neib_sparse.append(features[name_list[i]])
        feature_name_unpad.append(name_list[i]+"_unpad")
        feature_name_dense.append(name_list[i])
...
def network_fn(features, labels, mode, params):
    ...
    for i in range(len(feature_name_dense)):
        if i in {0, 3, 5, 7, 9, 10, 12, 14, 16}:    ## query_size
    ...
    query_mp1_embedding = tf.stack([query_embedding, neigb_embedding[0], neigb_embedding[1], neigb_embedding[2], neigb_embedding[3]], axis=1)
    query_mp1_embedding = tf.reduce_mean(query_mp1_embedding, axis=1)
    query_mp2_embedding = tf.stack([query_embedding, neigb_embedding[4], neigb_embedding[5], neigb_embedding[6], neigb_embedding[7]], axis=1)
    ...
    title_mp1_embedding = tf.stack([title_embedding, neigb_embedding[8], neigb_embedding[9], neigb_embedding[10], neigb_embedding[11]], axis=1)
    title_mp1_embedding = tf.reduce_mean(title_mp1_embedding, axis=1)
    title_mp2_embedding = tf.stack([title_embedding, neigb_embedding[12], neigb_embedding[13], neigb_embedding[14], neigb_embedding[15]], axis=1)
