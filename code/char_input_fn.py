...
class DataConfig(object):
    def __init__(self, config=None):
        self.train_dataset_files = "./train.tsv"
        self.eval_dataset_files = "./eval.tsv"
        self.test_dataset_files = "./test.tsv"
...
feature_names = ["q", "sna", "qiqiq1_t1", "qiqiq1_q2", "qiqiq2_t3", "qiqiq2_q4", "qiqiq1_t5", "qiqiq1_q6", "qiqiq2_t7", "qiqiq2_q8", "iqiqi1_q1", "iqiqi1_t2", "iqiqi2_q3", "iqiqi2_t4", "iqiqi1_q5", "iqiqi1_t6", "iqiqi2_q7", "iqiqi2_t8"]
...
def unigram_and_padding(string_tensor, width, padding_value):
    ...
    # sparse_tensor = tokenize_fn.unigrams_and_bigrams_parser(string_tensor)
...
def input_fn(filenames, batch_size, num_epochs, perform_shuffle, is_training=False):
    def decode_line(line):
        ...
        for i in range(3,11,1):
            if i % 2 == 1:
                neighbor.append(unigram_and_padding(columns.values[i], data_config.title_size, data_config.padding_string))
            else:
                neighbor.append(unigram_and_padding(columns.values[i], data_config.query_size, data_config.padding_string))
        for i in range(11,19,1):
            if i % 2 == 1:
                neighbor.append(unigram_and_padding(columns.values[i], data_config.query_size, data_config.padding_string))
            else:
                neighbor.append(unigram_and_padding(columns.values[i], data_config.title_size, data_config.padding_string))
        data_list = [query_ids, title_ids]
        print("111111length: ", len(neighbor))
        for i in range(16):
            data_list.append(neighbor[i])
        return dict(zip(feature_names, data_list)), labels
