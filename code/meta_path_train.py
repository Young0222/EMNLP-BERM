...
query_nei = {}
title_nei = {}
input_data='./new_train'
output_data='./train_nei_4order.tsv'
num_click = 0
num_purchase = 0
...
with io.open(input_data, 'r', encoding='utf-8') as f:
    for i, line1 in enumerate(f):
        ...
        title = line[2]
        score = float(line[5])
        num_click += float(line[3])
        num_purchase += float(line[4])
        if len(title) == 0:
...
print("num_purchase: ", num_purchase)
